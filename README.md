# D_personal-api-app

외부 API를 위젯으로 연결해 보고(텍스트·테이블·시계열) 보내는(HTTP 요청) 개인용 모바일 대시보드 앱. Expo / React Native.

## 문서
| 영역 | 위치 |
|---|---|
| 요구사항 (NB → PRD → SRS) | [docs/requirements](docs/requirements) |
| 아키텍처 (arc42, C4, ADR) | [docs/architecture](docs/architecture) |
| UX (User Flow, Wireframe) | [docs/ux](docs/ux) |
| 월간 일지 | [docs/journal](docs/journal) |
| 학습 노트 | [docs/learning](docs/learning) |

## 개발 방식
이슈 단위로 Claude Code와 대화형 진행, 규칙은 [CLAUDE.md](CLAUDE.md) 참고.

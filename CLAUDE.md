...
3. 같은 PR에 문서 동기화를 포함한다.
   ...
   - 일지: 그날 첫 PR부터 `journal` 스킬로 오늘 일지 갱신(하단 "개발자 퀵 가이드": 시행착오를 뺀, 실제로 효과 있었던 진행사항·명령·코드만 압축)
...
6. 파일을 건드린 작업은 **항상 파일 변경 요약**(추가/수정/삭제와 각 파일의 역할)을 보고에 포함한다. `git diff --stat`·`git status` 실제 결과를 근거로 쓴다.

### 머지 등급
| 등급 | 대상 | 처리 |
|---|---|---|
| 🟢 자체 머지 | 문서만 변경: `docs/journal`, `docs/learning`, `docs/reports`, README, 추적 문서의 번호·체크 반영 / 사용자가 결과를 확인하고 머지를 요청한 PR / 테스트만 추가, 동작 변경 없는 lint·포맷 수정 / **의존성 추가·마이너 업데이트** / 머지 충돌 해결(내용 변경 없이 양쪽 유지) | CI 통과 확인 후 머지 + 요약 보고 |
| 🟡 사용자 머지 | 앱 동작·화면 변경, 의존성 삭제·메이저 업그레이드, PRD·SRS 내용(범위·AC) 변경, ADR 신규·변경 | PR까지 만들고 확인할 점(실기기 확인 방법 포함)을 보고 |

...
| 등급 | 대상 | 처리 |
|---|---|---|
| 🟢 AI 자체 머지 | 문서만 변경(일지·학습 노트·리포트·README·추적표 번호), 사람이 확인 후 머지를 요청한 PR, (CI 도입 후) 테스트만 추가·동작 무변경 lint | CI 통과 확인 → 머지 → 요약 보고 |
| 🟡 사람 머지 | 앱 동작·화면 변경, 의존성 변경, PRD·SRS 내용 변경, ADR | PR + 확인할 점 보고 |
...
- 머지 등급:
  - 🟢 AI 자체 머지: 문서만 변경, 사용자가 확인 후 머지를 요청한 PR, (CI 도입 후) 테스트만 추가·동작 무변경 lint → CI 통과 확인 후 머지
  - 🟡 사용자 머지: 앱 동작·화면, 의존성, PRD·SRS 내용, ADR → PR과 확인할 점 보고

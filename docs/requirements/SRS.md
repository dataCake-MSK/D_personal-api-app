...
### SRS-020 글자 크기 설정
- 추적: PRD-008 · 이슈: #37
- 내용: 위젯 설정에 글자 크기(작게/보통/크게) 선택. 공통 텍스트 스타일 함수로 위젯 렌더러에 적용. (D5 결정에 따라 앱 전체 설정 추가 가능)
- AC:
...
- 추적: PRD-011 · 이슈: #39
- 내용: `expo-image-picker`로 기기 사진 선택 → 앱 저장소로 복사 후 경로를 설정에 보관(D6 결정 반영). 위젯 삭제 시 복사본 정리.
- AC:
  - [ ] 사진을 고르면 위젯에 표시되고 앱 재시작 후에도 남는다
  - [ ] 위젯을 삭제하면 복사본도 지워진다 (단위 테스트)
  - [ ] 실기기 확인
...
### SRS-024 이벤트 알림
- 추적: PRD-012 · 이슈: #41
- 내용: 위젯별 조건(숫자 임계값 초과/미만, 요청 실패, 이벤트 수신) 설정과 알림 표시. 1단계는 앱 사용 중 알림(D7).
- AC:
